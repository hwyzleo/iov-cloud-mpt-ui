    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['completeVehicle:transport:carrier:add']"
        >新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['completeVehicle:transport:carrier:edit']"
        >修改
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['completeVehicle:transport:carrier:remove']"
        >删除
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['completeVehicle:transport:carrier:export']"
        >导出
        </el-button>
      </el-col>
...
import {
  ...
  updateCarrier,
  delCarrier
} from "@/api/completevehicle/transport/carrier";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCarrier(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const carrierId = row.id || this.ids
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const carrierIds = row.id || this.ids;
      this.$modal.confirm('是否确认删除承运商ID为"' + carrierIds + '"的数据项？').then(function () {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('otd-tms/mpt/carrier/export', {
        ...this.queryParams
